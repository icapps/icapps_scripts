...
unless background_color = ARGV[0]
...
# Check if there is a fill color
unless fill_color = ARGV[1]
    fill_color = "black"
end
if fill_color =~ /\d/
...
for asset in xcassets["images"]
    ...
    `convert -background "#{background_color}" -fill "#{fill_color}" -gravity center -size #{width}x#{height} caption:"#{version}" "#{base_file}" +swap -gravity south -composite "#{target_file}"`
end